...
### 3-2. 비밀번호 최소 길이 (메뉴 없음)

Supabase **Cloud** 대시보드에는 비밀번호 **최소 길이를 4로 바꾸는 메뉴가 없습니다.**  
기본 최소 길이는 **6자리**이므로, SAFE 앱에서는 **교사/학생 비밀번호를 숫자 6자리**로 사용합니다.  
별도 설정 없이 그대로 사용하시면 됩니다.

(Self-hosted Supabase를 쓰는 경우에만 `GOTRUE_PASSWORD_MIN_LENGTH=4` 환경 변수로 4자리 허용이 가능합니다.)

### 3-3. 이메일 인증 끄기 (쉽게 가입하려면 필수)
