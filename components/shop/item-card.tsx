    ...
    >
      ...
      <div className="relative aspect-[5/3] w-full overflow-hidden bg-gradient-to-b from-amber-50 to-amber-100/40">
        {item.image_url ? (
          ...
          <div className="flex h-full items-center justify-center pb-1 text-7xl" aria-hidden>
            🎁
          </div>
        )}
        {owned && (
          <div className="absolute inset-0 flex items-center justify-center bg-emerald-900/35">
            <span className="rounded-full bg-emerald-300 px-3 py-1 text-xs font-extrabold text-emerald-950 shadow-sm">
              ✓ 보유
            </span>
          </div>
        )}
        <div
          className="absolute top-2 right-2 rounded-full px-2.5 py-1 text-xs font-extrabold shadow-sm"
          style={{
      ...
      <div className="p-3.5">
        <h3 className="font-extrabold text-gray-900 text-base leading-snug line-clamp-2">{item.name}</h3>
        <p className="text-sm font-medium text-gray-500 mt-0.5">{item.type}</p>
        {error && (
        ...
        {!owned && (
          <button
            ...
            className="mt-3 w-full rounded-xl py-2.5 text-base font-extrabold shadow-sm hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40 transition"
            style={{
